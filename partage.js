/* =============================================================
   ...
   Intégration :
     <script src="partage.js"></script>   (avant ton script principal)
     puis, sur l'écran de résultat :
       Partage.monter(conteneur, { jour, diff, resultats });
...
const Partage = (() => {
  ...
  .qh-case{
    width:20px;height:20px;border-radius:5px;background:#FF5C6E;
    opacity:0;transform:scale(.4);
    animation:qh-pop .32s cubic-bezier(.34,1.56,.64,1) forwards;
  }
  .qh-case.ok{ background:#3EE0AE; }
  ...
  .qh-bouton{
    ...
    color:#1B0F1E;background:#3EE0AE;border:none;border-radius:999px;
    padding:13px 30px;cursor:pointer;
    transition:transform .12s ease,box-shadow .18s ease,background .18s ease;
    box-shadow:0 4px 18px rgba(62,224,174,.22);
  }
  .qh-bouton:hover{ transform:translateY(-1px);box-shadow:0 6px 24px rgba(62,224,174,.32); }
  .qh-bouton:active{ transform:translateY(1px); }
  .qh-bouton:focus-visible{ outline:2px solid #3EE0AE;outline-offset:3px; }
  .qh-bouton[data-etat="fait"]{ background:#2A9A78;color:#EDE6F0; }
  .qh-note{
    font-family:'JetBrains Mono',ui-monospace,monospace;
    font-size:11px;letter-spacing:.06em;text-transform:uppercase;
    color:#8A7590;margin:0;min-height:14px;
  }`;
